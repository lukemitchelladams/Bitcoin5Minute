...
def build_chart(ind):
    ...
    fig.add_trace(go.Scatter(x=ts,y=ind["bb_dn_s"],name="BB-",fill="tonexty",
        ...
        line=dict(color="rgba(100,150,255,0.4)",width=1,dash="dot")),row=1,col=1)
    # Volume — split into two traces to avoid rgba list bug in plotly on Python 3.13
    up_mask = df["close"] >= df["open"]
    fig.add_trace(go.Bar(x=ts[up_mask],  y=df["volume"][up_mask],  name="Vol↑", marker_color="#00e676", opacity=0.5, showlegend=False), row=2, col=1)
    fig.add_trace(go.Bar(x=ts[~up_mask], y=df["volume"][~up_mask], name="Vol↓", marker_color="#ff5252", opacity=0.5, showlegend=False), row=2, col=1)
    # MACD histogram — same split approach
    hist_s = pd.Series(ind["hist_s"], index=range(len(ind["hist_s"])))
    pos_mask = hist_s >= 0
    fig.add_trace(go.Bar(x=ts[pos_mask],  y=hist_s[pos_mask],  name="Hist+", marker_color="#00e676", opacity=0.7, showlegend=False), row=3, col=1)
    fig.add_trace(go.Bar(x=ts[~pos_mask], y=hist_s[~pos_mask], name="Hist-", marker_color="#ff5252", opacity=0.7, showlegend=False), row=3, col=1)
